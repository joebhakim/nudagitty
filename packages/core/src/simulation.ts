...
type FastSampler = (rng: () => number) => number;
...
interface CompiledModel { plan: CompiledNodePlan[]; index: Map<string, number> }

function compileModel(graph: GraphModel, spec: SimulationSpec, order: string[]): CompiledModel | null {
...
function runCompiledForward(compiled: CompiledModel, spec: SimulationSpec, rng: () => number, sampleCount: number, intervention?: CompiledIntervention): { samples: Record<string, number[]>; lastValues: Record<string, number> } {
