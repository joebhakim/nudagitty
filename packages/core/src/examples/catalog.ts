...
export const EXAMPLES: ExampleModel[] = [
  ...
  Treatment [exposure,label="treatment",pos="-1.3,0.2"]
  Death [outcome,label="death",pos="1.3,0.8"]
  ...
  },
  {
    ...
    domain: "dgm",
    summary: "The two-part fitted-DGP positive control, with an HONEST marginal. Earnings_78 is SEMICONTINUOUS — a participation gate x a positive amount — so the fitted marginal reproduces the real earnings ZERO SPIKE and cannot pay anyone negative money. Earnings HISTORY enters the intensive margin through sqrt(), not dollars: a log link fed DOLLAR-VALUED history makes E[Y|L] exponential in dollars, which used to manufacture $2.6M earners (real max: $121k) and skew 31 (real: 1.3), and made an analyst's OLS report +$14,599 where the real rows give +$752. We were punishing estimators for a world we invented. Why sqrt and not the textbook Mincer log? Because the app's OWN residual test says log(1+x) is the WORST option here — dCor 0.354, worse even than raw dollars (0.245) — since a third of these rows have NO 1974 earnings and log blows that zero-mass up into a violent jump. sqrt scores 0.240 and fixes the tail just as well. That is the practitioner loop working: the residual panel names the offending predictor, you change its function, the panel re-scores. Now: simulated mean $21.0k / skew 2.9 (real $20.5k / 1.3), crude gap -$15.3k (real: -$15.6k), and the imposed +$1,794 is recovered by NOBODY — OLS +$3.6k, +interactions +$2.8k, cubic basis +$2.6k (closest), two-part -$5.7k, PPML -$7.1k. The family-aware rungs do WORST, because they fit log(Y) on RAW dollar regressors and then exponentiate, amplifying their own misspecification. Getting the FAMILY right buys nothing if the PREDICTOR SCALE is wrong. A hard, honest benchmark.",
    code: `dag {
  Row_source [latent,label="LaLonde rows (resample)",pos="0.0,4.4"]
  Age [adjusted,label="age",pos="-3.3,2.7"]
  Education [adjusted,label="education",pos="-2.35,3.15"]
