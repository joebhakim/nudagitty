...
export const EXAMPLES: ExampleModel[] = [
  ...
  },
  {
    ...
    domain: "dgm",
    summary: "The two-part upgrade of the fitted-DGP positive control, now with an HONEST marginal. Earnings_78 is SEMICONTINUOUS — a participation gate (did they earn anything?) x a positive amount (how much, if so) — so the fitted marginal reproduces the real earnings ZERO SPIKE that the linear/Gaussian version (lalonde-fit-recover) cannot, and cannot pay anyone negative money. Earnings HISTORY enters the intensive margin as a LOG (the Mincer specification): feeding dollar-valued history into a log link makes E[Y|L] exponential IN DOLLARS, which used to manufacture $2.9M earners (real max: $121k) and skew 33 (real: 1.3) — and made an analyst's OLS report +$14,599 where the real rows give +$752. We were punishing estimators for a world we invented. Fixed, the simulated marginal lands at mean $20.4k / skew 2.6 against a real $20.5k / 1.3. The confounding is still LEARNED from the real LaLonde rows; the +$1,794 benchmark is still IMPOSED (62% extensive-led). AND NOW NOTHING RECOVERS IT: the crude gap is -$13.9k, OLS -$1.9k, two-part -$7.8k, PPML -$7.7k. The family-aware rungs do WORSE than plain OLS, because they fit log(Y) on RAW dollar regressors and then exponentiate, so their misspecification is amplified. Getting the FAMILY right buys nothing if the PREDICTOR SCALE is wrong — that is the Mincer lesson landing on the estimator side, and the missing capability is rung 4 of the outcome-model ladder. Only relaxing the confounder basis to cubic gets close (+$2,259). This is what a hard, honest benchmark looks like.",
    code: `dag {
  Row_source [latent,label="LaLonde rows (resample)",pos="0.0,4.4"]
  Age [adjusted,label="age",pos="-3.3,2.7"]
  Education [adjusted,label="education",pos="-2.35,3.15"]
